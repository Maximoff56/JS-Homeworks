...
while (true) {
    ...
    if (count.length >= 1 && count[count.length - 1] === "1") {
        console.log(`На складе в наличии ${count} товар`);
        break;
    }
    if (count.length >= 1 && (count[count.length - 1] === "2" || count[count.length - 1] ==="3" || count[count.length - 1] === "4")) {
        console.log(`На складе в наличии ${count} товарa`);
        break;
    }
